...
def init_firebase():
    ...
    try:
        ...
        if not os.path.exists(cred_path):
            ...
            cred_path = os.path.join(os.path.dirname(BASE_DIR), 'serviceAccountKey.json')
            
        if os.path.exists(cred_path):
            logging.info(f"Using service account key: {cred_path}")
            ...
            logging.info("No serviceAccountKey.json found, trying ADC...")
        ...
        # Reduced logging level for common init warnings in development/local mode
        logging.info(f"Firebase Local Mode: {e}")
        # Ensure fs and bucket are None to be handled by routes
        fs = None
        bucket = None
...
class Client(db.Model):
    ...
    name = db.Column(db.String(100), nullable=True) # Добавлено
    hwid = db.Column(db.String(100), nullable=True) # Добавлено
...
with app.app_context():
    try:
        db.create_all()
        # Add missing columns for Client if they don't exist
        try:
            from sqlalchemy import text
            with db.engine.connect() as conn:
                columns = [c[1] for c in conn.execute(text("PRAGMA table_info(client)")).fetchall()]
                if 'name' not in columns:
                    conn.execute(text("ALTER TABLE client ADD COLUMN name VARCHAR(100)"))
                if 'hwid' not in columns:
                    conn.execute(text("ALTER TABLE client ADD COLUMN hwid VARCHAR(100)"))
                conn.commit()
        except Exception as e:
            logging.warning(f"Client migration warning: {e}")
...
def get_images():
    if not fs:
        return jsonify({'msg': 'Firebase Firestore не инициализирован'}), 503
    try:
...
@jwt_required()
def upload_image():
    if not fs or not bucket:
        return jsonify({'msg': 'Firebase не инициализирован'}), 503
...
@app.route('/api/cleanup/ram', methods=['POST'])
@jwt_required()
def cleanup_ram():
    try:
        # Для веб-панели мы можем реализовать очистку памяти самого сервера
        # или отправить команду клиентам (если реализована система команд)
        # В данном контексте, скорее всего, имеется в виду очистка на сервере
        import ctypes
        ctypes.windll.psapi.EmptyWorkingSet(ctypes.windll.kernel32.GetCurrentProcess())
        log_action("Server RAM Cleanup performed")
        return jsonify({'msg': 'Очистка ОЗУ сервера выполнена успешно'})
    except Exception as e:
        logging.error(f"RAM cleanup error: {e}")
        return jsonify({'msg': f'Ошибка очистки ОЗУ: {str(e)}'}), 500
            ...
            if folder and os.path.exists(folder):
                for filename in os.listdir(folder):
                    ...
                    try:
                        ...
                        continue
        log_action(f"Server Temp Cleanup performed: {count} items removed")
        return jsonify({'msg': f'Очистка временных файлов выполнена. Удалено объектов: {count}'})
    except Exception as e:
        logging.error(f"Temp cleanup error: {e}")
        return jsonify({'msg': f'Ошибка очистки временных файлов: {str(e)}'}), 500
