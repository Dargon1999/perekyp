...
from gui.widgets.utility_widgets import MemUtilityWidget, TempUtilityWidget
import sys
import os
...
class SettingsTab(QWidget):
    ...
    def showEvent(self, event):
        """Apply theme when settings tab is opened."""
    ...
    def setup_ui(self):
        ...
        tabs = [
            ...
            (5, "Интерфейс", "gui/assets/icons/display.svg"),
            (6, "Информация", "gui/assets/icons/info.svg")
        ]
        ...
        self.create_interface_page()    # Index 5
        self.create_info_page()         # Index 6
    ...
    def apply_theme(self, theme=None):
        ...
        # Update navigation styles to ensure visibility
        self.update_nav_styles()
    ...
    def create_general_page(self):
        ...
        ui_group = QGroupBox("Интерфейс и масштабирование")
        ui_layout = QVBoxLayout(ui_group)
        ui_layout.setSpacing(15)
        
        # Scaling Mode Toggle
        scaling_row = QHBoxLayout()
        scaling_row.addWidget(QLabel("Режим автоматического масштабирования:"))
        scaling_row.addStretch()
        
        self.scaling_toggle = ToggleSwitch()
        is_auto_scaling = self.data_manager.get_setting("auto_scaling", True)
        self.scaling_toggle.setChecked(is_auto_scaling)
        self.scaling_toggle.toggled.connect(self.on_scaling_toggled)
        scaling_row.addWidget(self.scaling_toggle)
        ui_layout.addLayout(scaling_row)
        
        scaling_info = QLabel("Автоматически подстраивает размер элементов под разрешение экрана.")
        scaling_info.setStyleSheet("color: #9ca3af; font-size: 11px;")
        ui_layout.addWidget(scaling_info)
        
        # Reset Window Size Button
        reset_size_row = QHBoxLayout()
        self.btn_reset_size = QPushButton("Сбросить размер окна (900x720)")
        self.btn_reset_size.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_reset_size.setFixedHeight(35)
        self.btn_reset_size.clicked.connect(self.on_reset_window_size)
        reset_size_row.addWidget(self.btn_reset_size)
        reset_size_row.addStretch()
        ui_layout.addLayout(reset_size_row)
        
        layout.addWidget(ui_group)
        
        # --- Notifications Section ---
        notif_settings_group = QGroupBox("Настройки уведомлений")
        notif_settings_layout = QVBoxLayout(notif_settings_group)
        notif_settings_layout.setSpacing(10)
        
        # Notification Duration
        dur_layout = QHBoxLayout()
        dur_layout.addWidget(QLabel("Длительность уведомлений (сек):"))
        self.notif_dur_input = QLineEdit()
        self.notif_dur_input.setFixedWidth(60)
        self.notif_dur_input.setText(str(self.data_manager.get_global_data("notif_duration", 3)))
        self.notif_dur_input.textChanged.connect(self.on_notif_dur_changed)
        dur_layout.addWidget(self.notif_dur_input)
        dur_layout.addStretch()
        notif_settings_layout.addLayout(dur_layout)
        
        # Audio Volume
        vol_layout = QHBoxLayout()
        vol_layout.addWidget(QLabel("Громкость звуков уведомлений:"))
        vol_layout.addStretch()
        self.vol_combo = QComboBox()
        self.vol_combo.addItems(["Выкл", "20%", "50%", "70%", "100%"])
        current_vol = self.data_manager.get_global_data("notif_volume", 70)
        vol_map = {0: 0, 20: 1, 50: 2, 70: 3, 100: 4}
        self.vol_combo.setCurrentIndex(vol_map.get(current_vol, 3))
        self.vol_combo.currentIndexChanged.connect(self.on_notif_vol_changed)
        vol_layout.addWidget(self.vol_combo)
        notif_settings_layout.addLayout(vol_layout)
        
        layout.addWidget(notif_settings_group)
    ...
    def update_storage_path_info(self):
        ...
        if not hasattr(self, 'storage_path_input'):
            return
        ...
        try:
            ...
            if hasattr(self, 'path_status_lbl'):
                self.path_status_lbl.setText(f"❌ Ошибка проверки пути: {str(e)}")
                self.path_status_lbl.setStyleSheet("color: #e74c3c; font-size: 11px;")
            logging.error(f"Error in update_storage_path_info: {e}")
    ...
    def on_palette_color_selected(self, color_hex):
        """Update accent color and apply theme with safety."""
        try:
            ...
            logging.error(f"Error in on_palette_color_selected: {e}")

    def on_header_customization_changed(self):
        """Update header colors and visibility with safety."""
        try:
            ...
            logging.error(f"Error in on_header_customization_changed: {e}")
    ...
    def on_startup_tab_changed(self, index):
        try:
            ...
            logging.error(f"Error in on_startup_tab_changed: {e}")
    ...
    def on_notif_combo_changed(self, index):
        try:
            ...
            logging.error(f"Error in on_notif_combo_changed: {e}")
    ...
    def create_advanced_page(self):
        ...
        # 3. Mem Sub-tab
        self.mem_utility_widget = MemUtilityWidget(self.data_manager, self.security_manager)
        self.advanced_tabs.addTab(self.mem_utility_widget, "Mem")
        
        self.advanced_tabs.currentChanged.connect(self.on_advanced_tab_changed)
        
        layout.addWidget(self.advanced_tabs)
        self.content_stack.addWidget(page)

    def create_hotkeys_startup_widget(self):
        widget = QWidget()
        ...
        # Autostart Section
        startup_group = QGroupBox("Автозагрузка")
        startup_layout = QVBoxLayout(startup_group)
        
        row = QHBoxLayout()
        row.addWidget(QLabel("Запускать с Windows (Права администратора):"))
        row.addStretch()
        
        self.startup_toggle = ToggleSwitch()
        is_startup = self.data_manager.get_setting("admin_autostart_enabled", False)
        self.startup_toggle.setChecked(is_startup)
        self.startup_toggle.toggled.connect(self.on_startup_toggled)
        row.addWidget(self.startup_toggle)
        startup_layout.addLayout(row)
        
        hint = QLabel("Используется Планировщик задач для обхода UAC при старте.")
        ...
        # F7 Config
        f7_row = QHBoxLayout()
        f7_row.addWidget(QLabel("Очистка RAM (F7):"))
        f7_row.addStretch()
        self.f7_input = QLineEdit(self.get_hotkey_for("mem_cleanup"))
        self.f7_input.setFixedWidth(60)
        self.f7_input.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.f7_input.textChanged.connect(lambda t: self.on_hotkey_changed("mem_cleanup", t))
        f7_row.addWidget(self.f7_input)
        hotkeys_layout.addLayout(f7_row)
        
        # F8 Config
        f8_row = QHBoxLayout()
        f8_row.addWidget(QLabel("Очистка файлов (F8):"))
        f8_row.addStretch()
        self.f8_input = QLineEdit(self.get_hotkey_for("temp_cleanup"))
        self.f8_input.setFixedWidth(60)
        self.f8_input.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.f8_input.textChanged.connect(lambda t: self.on_hotkey_changed("temp_cleanup", t))
        f8_row.addWidget(self.f8_input)
        hotkeys_layout.addLayout(f8_row)
        
        layout.addWidget(hotkeys_group)
        layout.addStretch()
        return widget

    def create_diagnostics_widget(self):
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setContentsMargins(20, 20, 20, 20)
        layout.setSpacing(15)
        ...
        # Hotkey Repair
        repair_group = QGroupBox("Устранение проблем")
        repair_layout = QVBoxLayout(repair_group)
        
        repair_btn = QPushButton("Переподключить горячие клавиши")
        repair_btn.clicked.connect(self.on_repair_hotkeys)
        repair_layout.addWidget(repair_btn)
        
        hint = QLabel("Нажмите, если F7/F8 перестали реагировать. Это перерегистрирует хуки в ОС.")
        hint.setStyleSheet("color: #7f8c8d; font-size: 11px;")
        repair_layout.addWidget(hint)
        layout.addWidget(repair_group)

        # Logs View
        logs_group = QGroupBox("Журналы диагностики")
        logs_layout = QVBoxLayout(logs_group)
        
        self.diag_log_view = QTextEdit()
        self.diag_log_view.setReadOnly(True)
        self.diag_log_view.setStyleSheet("background: #1a1a1a; color: #00ff00; font-family: Consolas; font-size: 10px;")
        logs_layout.addWidget(self.diag_log_view)
        
        refresh_btn = QPushButton("Обновить логи")
        refresh_btn.clicked.connect(self.refresh_diag_logs)
        logs_layout.addWidget(refresh_btn)
        
        layout.addWidget(logs_group)
        return widget

    def on_advanced_tab_changed(self, index):
        """Auto-run internal utilities when their tab is selected."""
        widget = self.advanced_tabs.widget(index)
        if isinstance(widget, (MemUtilityWidget, TempUtilityWidget)):
            # Start cleaning logic immediately
            widget.run_utility()

    def on_startup_toggled(self, checked):
        from version import APP_ID
        sm = StartupManager(APP_ID)
        if checked:
            if not sm.is_running_as_admin():
                QMessageBox.warning(self, "Права администратора", "Для настройки автозагрузки через планировщик требуются права администратора.")
                self.startup_toggle.setChecked(False)
                return
            
            success = sm.setup_admin_autostart(True)
            if success:
                self.data_manager.set_setting("admin_autostart_enabled", True)
                QMessageBox.information(self, "Успех", "Автозагрузка с правами администратора настроена.")
            else:
                QMessageBox.critical(self, "Ошибка", "Не удалось создать задачу в планировщике.")
                self.startup_toggle.setChecked(False)
        else:
            sm.setup_admin_autostart(False)
            self.data_manager.set_setting("admin_autostart_enabled", False)
    ...
    def on_scaling_toggled(self, checked):
        self.data_manager.set_setting("auto_scaling", checked)
        if self.main_window:
            self.main_window.apply_styles()
            
    def on_reset_window_size(self):
        if self.main_window:
            self.main_window.resize(900, 720)
            # Center it
            screen = QApplication.primaryScreen()
            if screen:
                geo = self.main_window.frameGeometry()
                geo.moveCenter(screen.availableGeometry().center())
                self.main_window.move(geo.topLeft())
            
    def on_notif_dur_changed(self, text):
        try:
            val = int(text)
            if 1 <= val <= 30:
                self.data_manager.set_global_data("notif_duration", val)
        except ValueError:
            pass
            
    def on_notif_vol_changed(self, index):
        vol_map = {0: 0, 1: 20, 2: 50, 3: 70, 4: 100}
        vol = vol_map.get(index, 70)
        self.data_manager.set_global_data("notif_volume", vol)
        if self.main_window and hasattr(self.main_window, 'notification_manager'):
            self.main_window.notification_manager.audio_output.setVolume(vol / 100.0)
    ...
    def create_interface_page(self):
        ...
        # 🖥️ 1. Window Scaling & Size
        size_group = QGroupBox("Размер окна и масштабирование")
        ...
        resize_label = QLabel("Разрешить растягивание окна:")
        resize_label.setStyleSheet("font-size: 14px; color: #ecf0f1;")
        self.resize_toggle = QCheckBox()
        is_resizable = self.data_manager.get_setting("window_resizable", True)
        self.resize_toggle.setChecked(is_resizable)
        self.resize_toggle.stateChanged.connect(self.on_resize_toggle_changed)
        resize_row.addWidget(resize_label)
        resize_row.addStretch()
        resize_row.addWidget(self.resize_toggle)
        size_layout.addLayout(resize_row)

        hint_label = QLabel("Если включено, вы можете увеличивать и уменьшать окно программы, просто растягивая его за края.")
        hint_label.setStyleSheet("color: #95a5a6; font-size: 12px;")
        hint_label.setWordWrap(True)
        size_layout.addWidget(hint_label)

        # Reset Size Button
        reset_btn = QPushButton("Сбросить размер окна")
        ...
        """)
        reset_btn.clicked.connect(self.on_reset_window_size)
        size_layout.addWidget(reset_btn)

        content_layout.addWidget(size_group)

        # 📏 2. Interface Elements
        elements_group = QGroupBox("Элементы интерфейса")
        elements_group.setStyleSheet(size_group.styleSheet())
        elements_layout = QVBoxLayout(elements_group)
        
        # Sidebar Info
        sidebar_info = QLabel("• Ширина боковой панели: 220px (развернуто) / 60px (свернуто)\n"
                             "• Минимальный размер окна: 900x720px\n"
                             "• Окно авторизации: фиксированное 400x500px")
        sidebar_info.setStyleSheet("color: #bdc3c7; font-size: 13px; line-height: 1.5;")
        elements_layout.addWidget(sidebar_info)
    ...
    def on_resize_toggle_changed(self, state):
        enabled = state == Qt.CheckState.Checked.value
        self.data_manager.set_setting("window_resizable", enabled)
        if self.main_window:
            self.main_window.set_resizable(enabled)
            
    def on_reset_window_size(self):
        if self.main_window:
            self.main_window._apply_default_size()
            QMessageBox.information(self, "Интерфейс", "Размер окна сброшен к значениям по умолчанию.")
