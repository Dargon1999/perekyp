...
class MainWindow(ResizeMixin, QMainWindow):
    def __init__(self, auth_manager=None, data_manager=None):
        ...
        # Install event filter to catch mouse events for resizing from child widgets
    ...
    def eventFilter(self, obj, event):
        """Global event filter to handle resizing and dragging from any child widget."""
        ...
        if event.type() in [QEvent.Type.MouseButtonPress, QEvent.Type.MouseMove, QEvent.Type.MouseButtonRelease]:
            # Only handle events for this window or its children
            if not isinstance(obj, QWidget):
                return super().eventFilter(obj, event)
                
            is_child = (obj == self or self.isAncestorOf(obj))
            if not is_child:
                return super().eventFilter(obj, event)

            # Get position relative to MainWindow
            global_pos = QCursor.pos()
            pos = self.mapFromGlobal(global_pos)
            
            # 1. Handle Resizing
            edge = self._get_resize_edge(pos) if self._resizable else None
            ...
            # 2. Handle Window Dragging (Requirement 3: Removed from MainWindow, back to TitleBar)
            # Logic moved back to CustomTitleBar as requested by user
            pass
